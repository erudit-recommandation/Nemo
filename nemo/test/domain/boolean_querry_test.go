...
func TestQueryOnePhraseWithNoOperator(t *testing.T) {
	...
	respArangoQuery := fmt.Sprintf(`CONTAINS(s.text, "%v")`, query)
...
func TestQueryMultiplePhrase(t *testing.T) {
	...
	respArangoQuery := fmt.Sprintf(`CONTAINS(s.text, "%v") AND CONTAINS(s.text, "%v") AND CONTAINS(s.text, "%v")`,
...
func TestQueryWithSimpleOperation(t *testing.T) {
	...
	var tests = []struct {
		...
		{"le vol d'un oiseau AND Le chant d'une cygogne",
			...
			fmt.Sprintf(`CONTAINS(s.text, "%v") AND CONTAINS(s.text, "%v")`, "le vol d'un oiseau", "Le chant d'une cygogne"),
		},
		{"Le chat et le renard OR Le chien et le loup",
			...
			fmt.Sprintf(`CONTAINS(s.text, "%v") OR CONTAINS(s.text, "%v")`, "Le chat et le renard", "Le chien et le loup"),
		},
		...
		{"abcde NOT bar",
			...
			fmt.Sprintf(`CONTAINS(s.text, "%v") AND NOT CONTAINS(s.text, "%v")`, "abcde", "bar"),
		},
	}
...
func TestQueryWithCombinedOperation(t *testing.T) {
	...
	var tests = []struct {
		...
		{"le vol d'un oiseau AND Le chant d'une cygogne NOT FOO",
			...
			fmt.Sprintf(`CONTAINS(s.text, "%v") AND CONTAINS(s.text, "%v") AND NOT CONTAINS(s.text, "%v")`, "le vol d'un oiseau", "Le chant d'une cygogne", "FOO"),
		},
		{"Le chat et le renard OR Le chien et le loup AND a OR b NOT c",
			...
			fmt.Sprintf(`CONTAINS(s.text, "%v") OR CONTAINS(s.text, "%v") AND CONTAINS(s.text, "%v") OR CONTAINS(s.text, "%v") AND NOT CONTAINS(s.text, "%v")`,
